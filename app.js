...
async function hydrateWorkspace() {
  const [auditPayload, reportsPayload, activityPayload, runsPayload, agentsPayload, settingsPayload, healthPayload] = await Promise.all([api('/audit').catch(() => ({ events: [] })), api('/reports').catch(() => null), api('/ai/activity').catch(() => ({ events: [] })), api('/reconciliation/runs').catch(() => ({ runs: [] })), api('/ai/agents').catch(() => ({ agents: [] })), api('/settings').catch(() => null), api('/health').catch(() => null)]);
...
function stat(v,l,m,k,page){return `<div class="metric-item clickable-card" data-page-link="${page}"><div class="metric-value">${v}</div><div class="metric-label">${l}</div><div class="metric-meta ${k}">${m}</div><button class="why-action" data-why="${l}" data-why-value="${v}">Why?</button><span class="card-arrow">↗</span></div>`}
...
function activityChecklist(event){const text=`${event.title||'Clario event'}: ${event.detail||''}`.trim();const id=(text.match(/RZP-\d+/i)||[])[0]||'';return `<div class="activity-check ${event.kind==='warning'?'warn':''}"><label><input type="checkbox" ${event.kind==='done'?'checked':''} /><span class="check-mark">${event.kind==='warning'?'!':event.kind==='active'?'◉':'✓'}</span><span class="activity-copy"><b>${escapeHtml(event.title||'Clario event')}</b><small>${escapeHtml(event.detail||'Clario controller event')}</small></span><time>${new Date(event.time||Date.now()).toLocaleTimeString()}</time></label><div class="activity-actions"><button class="activity-ask" data-ask="${escapeHtml(text)}">Ask Clario</button>${id?`<button class="activity-investigate" data-investigate="${escapeHtml(id)}">Investigate</button>`:''}<button class="activity-report" data-report="${escapeHtml(text)}">Report</button></div></div>`}
...
function liveActivityPage(){const events=workspaceState.activity.length?workspaceState.activity:workspaceState.audit;const agents=workspaceState.agents;const agentCards=agents.map(a=>`<div class="agent-card"><span class="agent-dot"></span><div><b>${a.name}</b><small>${a.role}</small></div><strong>${a.executions||0}</strong></div>`).join('');const recent=(events.length?events:[{title:'No agent activity yet',detail:'Run reconciliation to start the controller.',kind:'active'}]).slice(0,6);return `<div class="page activity-page"><div class="eyebrow">WORKSPACE / AUTOMATION</div><div class="activity-title-row"><div><h1 class="page-title">AI Agent Activity</h1><p class="page-subtitle">Live, auditable work from every Clario agent.</p></div><span class="status green">CONNECTED</span></div>${agentCards?`<section class="agent-strip"><div class="eyebrow">AGENTS ONLINE</div><div class="agent-strip-grid">${agentCards}</div></section>`:''}<div class="activity-layout"><section class="card section-card activity-live-card"><div class="card-heading"><div><div class="card-title">Latest activity</div><div class="card-note">${events.length} events recorded</div></div><button class="btn btn-secondary" data-page-link="audit">Open audit trail</button></div><div class="activity-checklist">${recent.map(activityChecklist).join('')}</div></section><section class="card section-card activity-chat-card"><div class="eyebrow">CLARIO CONTEXT</div><h2>Ask about activity</h2><p class="card-note">Ask what an agent did, why it ran, or what needs attention.</p><div class="inv-chat-messages" id="activity-chat-messages"><div class="chat-msg assistant"><div class="chat-bubble">I can explain the latest agent activity.</div></div></div><div class="chat-composer inv-chat-composer"><input id="activity-chat-input" placeholder="Why did this agent run?" autocomplete="off"/><button id="activity-chat-send">Ask</button></div></section></div></div>`}
...
function liveReportsPage(){
  ...
  const matchRateDisplay=String(metrics.matchRate??0).endsWith('%')?metrics.matchRate:`${metrics.matchRate??0}%`;
...
function liveInvestigationPage(){
  ...
  const aiMode=workspaceState.health?.mistral?'MISTRAL LIVE':'DETERMINISTIC FALLBACK';
...
function render(page='overview'){
  ...
  if(page==='settings'){
    const health=workspaceState.health||{};
    const healthCard=document.createElement('section');
    healthCard.className='card section-card system-health-card';
    healthCard.innerHTML=`<div class="section-head"><div><div class="eyebrow">SYSTEM HEALTH</div><h2>Demo readiness</h2></div><span class="status ${health.ok?'green':'orange'}">${health.ok?'OPERATIONAL':'DEGRADED'}</span></div><div class="health-status-grid"><span class="${health.database==='connected'?'healthy':'fallback'}">● MongoDB ${health.database==='connected'?'Connected':'Demo fallback'}</span><span class="${health.mistral?'healthy':'fallback'}">● Mistral ${health.mistral?'Connected':'Deterministic fallback'}</span><span class="healthy">● AI Agents Online</span><span class="healthy">● Last reconciliation ${workspaceState.runs[0]?.startedAt?new Date(workspaceState.runs[0].startedAt).toLocaleString():'Not run yet'}</span></div>${health.mistral?'':'<p class="fallback-notice">Mistral unavailable. Clario is using deterministic investigation mode with transaction evidence, so the workflow remains fully usable.</p>'}`;
    content.appendChild(healthCard);
  }
  const assistantBar=document.querySelector('#assistant-bar');
  if(assistantBar) assistantBar.hidden=true;
  const globalChat=document.querySelector('#chat-panel');
  if(globalChat) globalChat.hidden=true;
...
function bindPageChats(){
  ...
  if(currentPage==='activity'){
    const container=document.querySelector('#activity-chat-messages');
    const input=document.querySelector('#activity-chat-input');
    const send=document.querySelector('#activity-chat-send');
    const ask=()=>{const value=input?.value.trim();if(!value)return;askClario(value,{open:false,container,context:{page:'activity'}});if(input)input.focus()};
    if(send)send.onclick=()=>{void ask()};
    if(input)input.onkeydown=event=>{if(event.key==='Enter'){event.preventDefault();ask()}};
  }
...
function openDrawer(id){
  ...
  d.innerHTML=`
    ...
    ${riskHtml || '<div class="drawer-section risk-empty"><label>RISK SCORE</label><div class="risk-score-row mini"><div class="risk-ring low">0</div><div class="risk-score-copy"><b>LOW RISK · 0/100</b><small>No exception signals detected.</small></div></div></div>'}
...
if(searchTrigger)searchTrigger.onclick=()=>{if(currentPage!=='reconciliation'&&currentPage!=='transactions')render('reconciliation');setTimeout(()=>{const input=document.querySelector('.search-input');input?.focus();input?.scrollIntoView({behavior:'smooth',block:'center'})},0)};
const bell=document.querySelector('.bell');
if(bell)bell.onclick=()=>toast('No new Clario notifications');
function openChat(){
...
async function askClario(messages,{open=true,context={},container=null}={}){
  ...
  const localInput=container?.parentElement?.querySelector('.chat-composer input')||container?.querySelector('.chat-composer input');
  ...
  try{
    ...
    chatTyping(false);
    appendChatMessage('assistant',`Something went wrong: ${error.message}`,container);
  }
}
