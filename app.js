...
function bindPageChats(){
  ...
  if(currentPage==='reports'){
    ...
    const ask=()=>{const value=input?.value.trim();if(!value)return;askClario(value,{open:false,container,context:{page:'reports'}});if(input)input.focus()};
    if(send)send.onclick=ask;
    if(input)input.addEventListener('keydown',event=>{if(event.key==='Enter')ask()});
  }
